  ...
  return (
    <aside class="sidebar">
      <div class="brand" style="margin-bottom: 2.5rem; padding: 0 0.5rem;">
        <div class="brand-logo" style="width: 28px; height: 28px; border-radius: 8px; background: linear-gradient(135deg, hsl(var(--primary)), #6366f1); color: white; font-size: 1rem;">w</div>
        <span class="brand-name" style="font-size: 1.1rem; font-weight: 700; letter-spacing: -0.02em;">wapp</span>
      </div>

      <button class="sidebar-add-btn"
        style="margin-bottom: 2rem; height: 42px; border-radius: 12px; background: hsl(var(--primary)); box-shadow: var(--shadow-md); transition: transform 0.2s, box-shadow 0.2s;"
      ...
      <nav class="nav-group" style="gap: 0.25rem;">
        <button 
          class="nav-item" 
          classList={{ active: state.activeTab === "all" }}
          onClick={() => actions.setActiveTab("all")}
          style="padding: 0.625rem 0.875rem; border-radius: 10px;"
        >
          <LayoutGrid size={18} />
          <span>All Apps</span>
        </button>
      ...
      <div style="margin-top: auto; padding: 0.5rem;">
        <div style="padding: 1rem; background: hsl(var(--muted) / 0.5); border-radius: 12px; border: 1px solid hsl(var(--border));">
           ...
           <p style="font-size: 0.65rem; color: hsl(var(--muted-foreground)); line-height: 1.4;">Press <kbd style="background: hsl(var(--background)); border: 1px solid hsl(var(--border)); padding: 0 0.2rem; border-radius: 3px;">⌘ K</kbd> to quickly create a new wapp.</p>
        </div>
      </div>
    </aside>
  );

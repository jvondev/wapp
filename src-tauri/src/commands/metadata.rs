...
pub async fn get_site_info(url: String) -> Result<SiteInfo, String> {
    ...
    let html_content = response.text().await.map_err(|e| format!("Failed to read response: {}", e))?;
    ...
    title = document.select(&og_title_selector).next().and_then(|m| m.value().attr("content")).map(|s| s.to_string());
    if title.is_none() {
        title = document.select(&title_selector).next().map(|t| t.inner_html());
    }
    ...
    for selector_str in icon_selectors {
        ...
        if let Some(link) = document.select(&sel).next() {
            let attr = if selector_str.starts_with("meta") { "content" } else { "href" };
            if let Some(raw) = link.value().attr(attr) {
                ...
                if icon_path.starts_with("//") {
                    ...
                    if let Ok(base) = url::Url::parse(&target_url) {
                        icon_path = format!("{}://{}{}", base.scheme(), base.host_str().unwrap_or_default(), icon_path);
                    }
    ...
    if icon.is_none() {
        if let Ok(base) = url::Url::parse(&target_url) {
            icon = Some(format!("{}://{}/favicon.ico", base.scheme(), base.host_str().unwrap_or_default()));
        }
    }

    if let Some(icon_url) = icon {
        if let Ok(icon_res) = reqwest::Client::new().get(&icon_url).header(reqwest::header::USER_AGENT, MODERN_USER_AGENT).send().await {
...
fn guess_mime(url: &str) -> &'static str {
    ...
    else if url.ends_with(".ico") { "image/x-icon" }

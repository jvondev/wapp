...
#[tauri::command]
async fn get_site_info(url: String) -> Result<SiteInfo, String> {
    ...
    // Simple parsing for icon (favicons)
    // We look for <link rel="icon" ... href="..."> or <link rel="shortcut icon" ...>
    let patterns = vec!["rel=\"icon\"", "rel=\"shortcut icon\"", "rel='icon'", "rel='shortcut icon'"];
    for pattern in patterns {
        if let Some(idx) = html.find(pattern) {
            // Search backwards for <link and forwards for href
